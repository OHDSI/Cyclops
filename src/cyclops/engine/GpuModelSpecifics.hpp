    ...
    virtual void computeRemainingStatistics() {
    	...
    	if (syncCV) {
			...
			if (BaseModelG::useNWeights) {
				...
				kernel.set_arg(10, dNtoK);
				kernel.set_arg(11, dNWeight);
				globalWorkSize[0] = syncCVFolds;
...
*/
        	...
        	/*
        	std::vector<real> blah;
        	blah.resize(dNWeight.size());
        	compute::copy(std::begin(dNWeight), std::end(dNWeight), std::begin(blah), queue);
        	std::cout << "dNWeight: ";
        	for (auto x:blah) {
        		std::cout << x << " ";
        	}
...
/*
        ...
        auto& kernel1 = kernelEmpty;
        kernel1.set_arg(0, 1);
    	queue.enqueue_1d_range_kernel(kernel1, 0, globalWorkSize, tpb);

#ifdef CYCLOPS_DEBUG_TIMING
        auto end1 = bsccs::chrono::steady_clock::now();
        ///////////////////////////"
        auto name1 = "emptyKernelG" + getFormatTypeExtension(formatType) + " ";
...
virtual void runCCDIndex() {
	...
	//for (int index = 0; index < J; index++) {
			...
			if (BaseModel::exactCLR) {
				kernel.set_arg(19, dNtoK);
				kernel.set_arg(20, dNWeight);
				int KStride = detail::getAlignedLength<16>(K);
				kernel.set_arg(21, KStride);
				int dN = N;
				kernel.set_arg(22, dN);
				globalWorkSize[1] = detail::constant::exactCLRBlockSize;
				localWorkSize[1] = detail::constant::exactCLRBlockSize;
			} else {
			...
			duration[name] += bsccs::chrono::duration_cast<chrono::TimingUnits>(end - start).count();
		        ...
		        */
		        ...
		        queue.enqueue_1d_range_kernel(kernel1, 0, globalWorkSize, localWorkSize);
		        queue.finish();
...
*/
    	...
    	if (reset && activeFolds > multiprocessors) {
...
*/
    ...
    SourceCode writeCodeForExactCLRDoItAllSingleKernel(FormatType formatType, int priorType);
