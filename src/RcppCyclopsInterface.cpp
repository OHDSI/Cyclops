...
namespace bsccs {
 ...
 static std::map<ModelType, std::string> modelTypeNames = {
	...
	{ModelType::EFRON_TIED_CONDITIONAL_LOGISTIC, "clr_efron"},
...
std::vector<std::string> cyclopsGetRemoveInterceptNames() {
	...
	std::vector<std::string> names = {
		...
		modelTypeNames[ModelType::EFRON_TIED_CONDITIONAL_LOGISTIC],
...
void cyclopsSetControl(SEXP inRcppCcdInterface,
        ...
        int maxBoundCount, const std::string& algorithm, bool useSyncCV, bool useLogSum, double mmtolerance
		) {
	using namespace bsccs;
	XPtr<RcppCcdInterface> interface(inRcppCcdInterface);
	// Convergence control
    ...
    args.modeFinding.mmtolerance = mmtolerance;
    args.modeFinding.useLogSum = useLogSum;
    if (algorithm == "mm") {
        args.modeFinding.algorithmType = AlgorithmType::MM;
    }
    if (algorithm == "ccdgreedy") {
    	args.modeFinding.algorithmType = AlgorithmType::CCDGREEDY;
    }
	...
	args.crossValidation.useSyncCV = useSyncCV;
