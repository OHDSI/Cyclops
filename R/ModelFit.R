...
.checkInterface <- function(x, computeDevice = "native", forceNewObject = FALSE, testOnly = FALSE) {
    if (forceNewObject
        ...
        || .cyclopsGetComputeDevice(x$cyclopsInterfacePtr) != computeDevice # was commented out in gpu
    ) {
...
createControl <- function(maxIterations = 1000,
    ...
    validAlgorithmNames = c("ccd", "mm", "ccdgreedy")
    stopifnot(algorithm %in% validAlgorithmNames)

    if (is.null(mmtolerance)) {
        mmtolerance = tolerance
    }
