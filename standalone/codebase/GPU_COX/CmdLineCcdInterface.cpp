...
//		reader = new NewGenericInputReader(modelType, logger, error);
		...
		reader = new NewCoxInputReader<float>(logger, error);
	} else {
		cerr << "Invalid file format." << endl;
		exit(-1);
	}
...
// 	}
    ...
    // TODO Cast ModelData<double> -> ModelData<float>

//    auto& doubleData = static_cast<ModelData<double>&>(**modelData);
//
//    *modelData = doubleData.castToFloat();

	*model = AbstractModelSpecifics::factory(modelType, **modelData,
			DeviceType::GPU, "Intel(R) Iris(TM) Plus Graphics 655");
	if (*model == nullptr) {
		cerr << "Invalid model type." << endl;
		exit(-1);
	}
